  ...
  getConfig() {
    ...
    return {
      ...
      memoryThresholdMB: process.env.LEAK_MEMORY_THRESHOLD_MB ? 
        parseInt(process.env.LEAK_MEMORY_THRESHOLD_MB) : envConfig.memoryThresholdMB,
      
      heapGrowthThreshold: process.env.LEAK_HEAP_GROWTH_THRESHOLD ?
        parseFloat(process.env.LEAK_HEAP_GROWTH_THRESHOLD) : envConfig.heapGrowthThreshold,
      
      verbose: process.env.LEAK_DETECTION_VERBOSE === 'true' || envConfig.verbose,
      
      generateHeapSnapshots: process.env.GENERATE_HEAP_SNAPSHOTS === 'true' || envConfig.generateHeapSnapshots
    };
  },
