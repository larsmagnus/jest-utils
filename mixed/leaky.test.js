...
describe('Leaky test simulation', () => {
  ...
  test('memory leak test - global variables', () => {
    ...
    global.testData = new Array(1000000).fill('leak'); // Large array
  ...
  test('event listener leak test', () => {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      globalEventListeners.push({ element: mockElement, type: 'click', handler });
    }
  ...
  test('well-behaved test', () => {
    ...
    const localData = new Array(100).fill('temporary');
  ...
  test('DOM-like leak test', () => {
    ...
    for (let i = 0; i < 20; i++) {
      const node = {
        ...
        children: [],
        parent: null,
        data: new Array(1000).fill(`node-${i}-data`)
      };
      mockNodes.push(node);
    }
