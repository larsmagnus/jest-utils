...
class LeakDetector {
  ...
  finishTest(testId, status) {
    ...
    if (this.options.generateHeapSnapshots && metrics.leaksDetected.length > 0) {
  ...
  analyzeMemoryLeak(metrics) {
    const memoryDiff = metrics.finalMemory.heapUsed - metrics.initialMemory.heapUsed;
    ...
    if (memoryDiffMB > this.options.memoryThresholdMB) {
      metrics.leaksDetected.push({
        type: 'MEMORY_LEAK',
        severity: 'HIGH',
        message: `Memory increased by ${memoryDiffMB.toFixed(2)}MB (threshold: ${this.options.memoryThresholdMB}MB)`,
        details: {
      ...
      metrics.warnings.push({
        type: 'MEMORY_GROWTH',
        severity: 'MEDIUM',
        message: `Heap grew by ${(heapGrowthRate * 100).toFixed(1)}% (threshold: ${this.options.heapGrowthThreshold * 100}%)`,
        details: {
          growthRate: heapGrowthRate,
  ...
  analyzeGlobalLeak(metrics) {
    ...
    const newGlobals = this.diffGlobalState(metrics.initialGlobals, metrics.finalGlobals);
  ...
  analyzeEventListenerLeak(metrics) {
    ...
    const listenerDiff = metrics.finalEventListeners.count - metrics.initialEventListeners.count;
  ...
  captureEventListenerState() {
    ...
    if (typeof window !== 'undefined' && window.document) {
      ...
      count = process.listenerCount ? Object.keys(process._events || {}).length : 0;
    }
  ...
  ensureHeapSnapshotDir() {
    try {
      ...
      this.log(`Error creating heap snapshot directory: ${error.message}`, 'ERROR');
    }
  }
  ...
  reportLeaks(metrics) {
    ...
    if (metrics.leaksDetected.length > 0) {
      ...
      metrics.leaksDetected.forEach((leak, index) => {
        this.log(`   ${index + 1}. [${leak.severity}] ${leak.type}: ${leak.message}`, 'ERROR');
        if (this.options.verbose && leak.details) {
          this.log(`      Details: ${JSON.stringify(leak.details, null, 2)}`, 'ERROR');
        }
      });
    }
    ...
    if (metrics.warnings.length > 0) {
      ...
      metrics.warnings.forEach((warning, index) => {
        this.log(`   ${index + 1}. [${warning.severity}] ${warning.type}: ${warning.message}`, 'WARN');
      });
    }
  ...
  getSummary() {
    ...
    return {
      ...
      totalLeaks: allMetrics.reduce((sum, m) => sum + m.leaksDetected.length, 0),
      ...
      worstOffenders: testsWithLeaks
        ...
        .map(m => ({
          ...
          memoryImpact: ((m.finalMemory.heapUsed - m.initialMemory.heapUsed) / (1024 * 1024)).toFixed(2)
        }))
    };
  }
