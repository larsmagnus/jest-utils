 ...
 * 
 * To use this setup:
 * 1. Add to jest.config.js: setupFilesAfterEnv: ['<rootDir>/jest-setup-leak-detection.js']
...
afterEach(() => {
  if (currentTestId) {
    ...
    const status = testState.assertionCalls === testState.expectedAssertionsNumber ? 'passed' : 'failed';
...
afterAll(() => {
  ...
  if (summary.totalLeaks > 0 || summary.totalWarnings > 0) {
    ...
    console.log(`🚨 Tests with Leaks: ${summary.testsWithLeaks} (${summary.totalLeaks} total leaks)`);
    console.log(`⚠️  Tests with Warnings: ${summary.testsWithWarnings} (${summary.totalWarnings} total warnings)`);
    ...
    if (summary.worstOffenders.length > 0) {
      ...
      summary.worstOffenders.forEach((offender, index) => {
        ...
        console.log(`      Leaks: ${offender.leakCount}, Memory Impact: ${offender.memoryImpact}MB`);
      });
    }
